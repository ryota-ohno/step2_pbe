...
from make_6_para import exec_gjf##計算した点のxyzfileを出す
from vdw_6_para import vdw_R##同様
...
def init_process(args):##使わない
    ...
    def get_init_para_csv(auto_dir,monomer_name):
        ...
        df_init_params = pd.DataFrame(np.array(init_para_list),columns = ['a','b','theta','R3','R4','status'])
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=3:##get Eの長さは計算した分子の数           ##ここの確認しっかり
            continue
        else:
            len_queue-=1;machine_type_list.remove(machine_type)
            Et1=float(E_list[0]);Et2=float(E_list[1]);Ep=float(E_list[2])#;Ep2=float(E_list[2])##6分子に向けてep1,ep2作成　ep1:b ep2:a
        ...
        for a in [a_init_prev-0.1,a_init_prev,a_init_prev+0.1]:
            for b in [b_init_prev-0.1,b_init_prev,b_init_prev+0.1]:
                for theta in [theta_init_prev-0.5,theta_init_prev,theta_init_prev+0.5]:
                    a = np.round(a,1);b = np.round(b,1);theta = np.round(theta,1)
                    df_val_ab = df_val[(df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&(df_val['status']=='Done')]
                    if len(df_val_ab)==0:
                        return False,{'a':a,'b':b,'theta':theta}
                    params_list.append([a,b,theta]);E_list.append(df_val_ab['E'].values[0])
        a_init,b_init,theta_init = params_list[np.argmin(np.array(E_list))]
